...
computePValues = function(MCListConfScores = NULL, testConfScores = NULL, seedval = NULL)
{
...
seedval = NULL
if (length(args) > 3) {
    seedval = as.integer(args[4])
}


fittedmodel <- readRDS("MODELS/model_carcinogenecity_pubchem.rds")
...
if (applyadan) {
    ...
    pValues = computePValues(MCListConfScores, testConfScores, seedval)
