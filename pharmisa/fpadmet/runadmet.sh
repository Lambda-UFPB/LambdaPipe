...
function usage() {
   echo "Usage: $0 [-f <smi file>] [-p <property>] [-h <help>] [-a <calculate prediction uncertainty>] [-s <seed>]"; exit $E_OPTERROR;
   echo "=================================================================="
...
while getopts "f:p:s:ah" opt; do
    case $opt in
        ...
        f)
            ...
            # Convert to an absolute path here since we will
            # cd into the program directory later.
            molfile="$(cd "$(dirname "$OPTARG")"; pwd)/$(basename "$OPTARG")"
        ;;
        p)
        ...
        s)
            ...
            seed=${OPTARG}
            if ! [[ "$seed" =~ ^[0-9]+$ ]]; then
        		echo "Seed values should be integers only."
        		exit $E_OPTERROR
			fi
        ;;
...
case $ptype in
    1)  echo "Anticommensal Effect on Human Gut Microbiota"
        ...
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_anticommensal.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    2)  echo "Blood-brain-barrier penetration"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_bbbp.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    3)  echo "Oral Bioavailability"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_bioavailability.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    4)  echo "AMES Mutagenecity"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_ames.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    5)  echo "Metabolic Intrinsic Clearance"
        ...
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_metabolicstability.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    6)  echo "Rat Acute LD50"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_rattoxicity.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    7)  echo "Drug-Induced Liver Inhibition"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_DILI.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    8)  echo "HERG Cardiotoxicity"
        ...
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_hergcardiotox.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    9)  echo "Haemolytic Toxicity"
        $FPGEN -output $FPOUT -fptype FCFP6 -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_hemotox.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    10) echo "Myelotoxicity"
        $FPGEN -output $FPOUT -fptype FCFP4 -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_myelotox.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    11) echo "Urinary Toxicity"
        $FPGEN -output $FPOUT -fptype FCFP4 -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_urinarytox.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    12) echo "Human Intestinal Absorption"
        ...
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_HIA.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    13) echo "Hepatic Steatosis"
        $FPGEN -output $FPOUT -fptype MACCS -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_steatosis.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    14) echo "Breast Cancer Resistance Protein Inhibition"
        $FPGEN -output $FPOUT -fptype FCFP4 -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_bcrpinhibition.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    15) echo "Drug-Induced Choleostasis"
        ...
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_choleostasis.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    16) echo "Human multidrug and toxin extrusion Inhibition" # MATE1
        ...
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_MATE1.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    17) echo "Toxic Myopathy"
        $FPGEN -output $FPOUT -fptype DFS -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_myopathy.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    18) echo "Phospholipidosis"
        ...
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_phospholipidosis.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    19) echo "Human Bile Salt Export Pump Inhibition"
        ...
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_BSEP.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    20) echo "Organic anion transporting polypeptide 1B1 binding"
        ...
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_OATP1B1.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    21) echo "Organic anion transporting polypeptide 1B3 binding"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_OATP1B3.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    22) echo "Organic anion transporting polypeptide 2B1 binding"
        $FPGEN -output $FPOUT -fptype ECFP6 -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_OATP2B1.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    23) echo "Phototoxicity human"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_phototoxhuman.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    24) echo "Phototoxicity invitro"
        ...
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_phototoxinvitro.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    25) echo "Respiratory Toxicity"
        $FPGEN -output $FPOUT -fptype MACCS -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_respiratorytox.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    26) echo "P-glycoprotein Inhibition"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_pgpinhibition.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    27) echo "P-glycoprotein Substrate"
        ...
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_pgpsubstrate.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    28) echo "Mitochondrial Toxicity"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_mitichondrialtox.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    29) echo "Carcinogenecity"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_carcinogenecity.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    30) echo "DMSO Solubility"
        ...
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_DMSO.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    31) echo "Human Liver Microsomal Stability"
        ...
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_HLMstability.R" $FPOUT $PREDOUT $adan $seed
    	else
    ...
    32) echo "Human Plasma Protein Binding"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_PPB.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    33) echo "hERG Liability"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_hergliability.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    34) echo "Organic Cation Transporter 2 Inhibition"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_OCT2inhibition.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    35) echo "Drug-induced Ototoxicity"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_ototoxicity.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    36) echo "Rhabdomyolysis"
        $FPGEN -output $FPOUT -fptype MACCS -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_rhabdomyolysis.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    37) echo "T1/2 Human"
        $FPGEN -output $FPOUT -fptype ASP -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_thalfhuman.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    38) echo "T1/2 Mouse"
        $FPGEN -output $FPOUT -fptype ECFP2 -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_thalfmouse.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    39) echo "T1/2 Rat"
        $FPGEN -output $FPOUT -fptype KR -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_thalfrat.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    40) echo "Cytotoxicity HepG2 cell lines"
        $FPGEN -output $FPOUT -fptype AT2D -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_cytotoxicityhepg2.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    41) echo "Cytotoxicity NIH-3T3 cell lines"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_cytotoxicitynih.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    42) echo "Cytotoxicity HEK 293 cell lines"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_cytotoxicityhek.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    43) echo "Cytotoxicity CRL-7250 cell lines"
        $FPGEN -output $FPOUT -fptype AT2D -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_cytotoxicitycrl.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    44) echo "Cytotoxicity HaCat cell lines"
        $FPGEN -output $FPOUT -fptype AT2D -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_cytotoxicityhacat.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    45) echo "CYP450 1A2 Inhibition"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_cyp1a2.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    46) echo "CYP450 2C19 Inhibition"
        $FPGEN -output $FPOUT -fptype ECFP4 -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_cyp2c19.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    47) echo "CYP450 2C9 Inhibition"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_cyp2c9.R" $FPOUT $PREDOUT $adan $seed
    	else
    ...
    48) echo "CYP450 2D6 Inhibition"
        $FPGEN -output $FPOUT -fptype FCFP4 -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_cyp2d6.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    49) echo "CYP450 3A4 Inhibition"
        $FPGEN -output $FPOUT -fptype FCFP6 -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_cyp3a4.R" $FPOUT $PREDOUT $adan $seed
        else
    ...
    57) echo "CYP450 2C8 Inhibition"
        $FPGEN -output $FPOUT -fptype PUBCHEM -mol $molfile
        if [ ! -z "$seed" ]; then
        	$RSCRIPT $PREDICTIONSCRIPTS"/predict_cyp2c8.R" $FPOUT $PREDOUT $adan $seed
        else
