...
class PharmitControlFirefox:
    ...
    def __init__(self, receptor_path: str, ligand_path: str, output_folder_path: str):
        ...
        self.no_results = []
        self.is_plip = None
        self.db_list = [['wuxi-lab', 'zinc', 'ultimate', 'molport', 'enamine'],
                        ['chembl', 'chemspace', 'pubchem', 'mcule', 'chemdiv']]
        ...
        profile.set_preference("browser.tabs.unloadOnLowMemory", False)
    ...
    def _open_tab(self, count,  db):
        ...
        self.window_handles[db] = self.driver.current_window_handle
    ...
    def _change_db(self, run, count, db):
        time.sleep(3)
        if run == 1:
            self.driver.switch_to.window(self.window_handles[self.db_list[0][count]])
        else:
            self.driver.switch_to.window(self.window_handles[db])
    ...
    def _upload_json(self, n, json_path, db, run):
        # Upload session
        self.driver.switch_to.window(self.window_handles[self.db_list[0][n]])
        # Mudar isso
        time.sleep(5)
        ...
        while True:
            ...
            try:
                ...
                pass
            try:
                if current_upload_time - start_upload_time > 180:
    ...
    def _search(self):
        ...
        while True:
            try:
                ...
                pass
        while True:
            end_search_loop_2 = time.time()
            try:
                self.driver.find_element(By.XPATH, '//*[@class="pharmit_namecol sorting_disabled"]')
                break
            except NoSuchElementException:
                if end_search_loop_2 - start_search_loop > 60 and not already_clicked:
                    search.click()
    ...
    def _run_chain(self, db, index, run_pharmisa, run, mode):
        ...
        if run_pharmisa == 0 and not (self.dbs_opened == current_db_list_size):
            ...
            self.dbs_opened += 1
        time.sleep(1)
        self._upload_json(n=index, json_path=self.modified_json_path, db=db, run=run)
    ...
    def run_pharmit_search(self, modified_json_path, run_pharmisa, quit_now=False, is_plip=None, fast=False):
        ...
        if PharmitControlFirefox.check_finished_download(self.minimize_count, old_download_list):
            if quit_now:
                self.driver.quit()
