...
def color_format(val):
    ...
    if float == type(val):
        ...
        color = '#2e7d32' if val == 'Accepted' else '#b71c1c'
        return f'background-color: {color}; font-weight: bold'


def get_df_html(df_html_old: pd.DataFrame):
    first_column = df_html_old.iloc[:, 0]
    regras = df_html_old.iloc[:, 25:29]
    toxicity = df_html_old.iloc[:, 2:13]
    df_html = pd.concat([first_column, toxicity, regras], axis=1)
...
def results_to_html(output_folder_path: str, folder_name: str):
    ...
    styled_html_df = df_html.style.map(color_format, subset=df_html.columns[1:16])
